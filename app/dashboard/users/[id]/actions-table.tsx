...
type UserAction = NonNullable<UserDetail["actions"]>[number];

interface ActionsTableProps {
  actions: UserAction[] | undefined;
}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <dd>
                <Button asChild variant="link" className="h-6 p-0 text-sm">
                  <Link href={`/dashboard/user-actions/${latestAction.id}`}>View details</Link>
                </Button>
              </dd>
            </div>
          </dl>
          ...
          <TableBody>
            {otherActions.map((action) => (
              <TableRow key={action.id}>
                ...
                <TableCell className="px-2 py-1">
                  <Button asChild variant="link" className="text-md -mx-4 -my-2 block w-full truncate font-normal">
                    <Link href={`/dashboard/user-actions/${action.id}`}>View details</Link>
                  </Button>
                </TableCell>
              </TableRow>
            ))}
